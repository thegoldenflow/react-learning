/**
 * 区块七：useState 的两个常见报错 —— react.dev useState 页 Troubleshooting 里的两条。
 * 1）Too many re-renders：渲染时无条件调用 setter（最常见的来源是 onClick={handleClick()}，把「传函数」写成了「调用函数」）；
 * 2）想把函数存进 state，结果它被调用了：useState / setter 收到函数时会把它当成初始化函数 / 更新函数。
