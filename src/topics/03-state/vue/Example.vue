 ...
 *          Vue 这一侧的演示（ref、reactive、直接改、computed 查找、存 id）都是 Vue 项目里常用的写法，没有注释掉的部分；React 那一侧【少用】的演示已注释。
 *
 * 完整的十段讲解在 react/Example.tsx；本文件列出 Vue 这一侧的要点。
 *
 * Vue 这一侧的要点：
 * - 声明响应式状态：【最常用】ref()（「In Composition API, the recommended way to declare reactive state is using the ref() function」）；
 *   【常用】reactive()（工程经验：一组相关字段放进一个对象、原地改，区块六的 QuantityEditor.vue）。
 ...
 *   只要 id 稳定，推荐存 id + computed 查找，两种情况都对得上（测试覆盖）。
 ...
 * - 把函数存进 ref【主流】：ref(fn) 存的就是 fn，Vue 不会把它当初始化函数调用（测试覆盖）；React 要写 useState(() => fn)（React 项目里少用，那边的演示已注释，见 react/Example.tsx 附 1）。
    ...
    <div class="card stack">
      ...
      <ul class="stack">
        <li>
          ...
          （React 有一个有条件的例外：记住上一次的 prop，少用，见 react/Example.tsx 附 2）。
        </li>
        <li>
