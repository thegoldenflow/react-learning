...
function PriceViews({ price }: { price: number }) {
  ...
  return (
    <ul className="stack">
      ...
      <li>
        ✅ 直接读 price：<strong data-testid="direct">{price}</strong>；含税价在渲染时算：<strong data-testid="with-tax">{withTax}</strong>
      </li>
    </ul>
  )
}
...
/**
 * 真想「只取初始值、之后自己改」（例如编辑草稿）时，就是有意忽略后续更新 —— 把 prop 命名成 initialX / defaultX 说明意图。
...
function PriceDraft({ initialPrice }: { initialPrice: number }) {
  ...
  return (
    <div className="row">
      <span>
        草稿价（initialPrice 只取初始值）：<strong data-testid="draft">{draft}</strong>
      </span>
