 ...
 * 使用频率：同一件事有几种写法时，按项目里的使用频率标【最常用】【常用】【少用】，并写出依据（官方原文、官方示例和主流库源码的写法、风格指南；没有出处的写「工程经验」）。
 *          正文和运行中的演示只有【最常用】【常用】的写法；【少用】的写法和只为说全的细节集中在文末「附」（本题的少用写法本来就没有单独的演示，不需要注释代码；
 *          区块二的 AmountEditor 为了演示 ❌ 直接改 props 用了整个 props 对象，那是反例的载体，不算少用写法的演示）。
 *          两种写法各有主流出处、分不出高低时写「两种都常见，按团队约定」。30 秒速答只用【最常用】。带频率标签的写法，成熟度是【主流】时不再重复标。
 ...
 * - 不要把 props 复制进 state：useState(props.x) 只在第一次渲染时取值；直接用 prop 或在渲染时计算【主流】。
 * - React 没有自动透传：自定义组件想接收原生属性，就在类型上继承 ComponentProps<'button'> 这类类型（@types/react 建议写成等价的 ComponentPropsWithRef<'button'>，本课 UiButton 就这样写），
 *   运行时把 {...rest} 展开到真实元素上，className / style 自己合并；
 ...
 *
 * 二、核心概念（React）
 * 1. props 是唯一的参数（区块一）：passing-props「props are the only argument to your component! React component functions accept a single argument,
 *    a props object」。读 props 按使用频率：
 *    - 【最常用】在参数里解构：「Usually you don't need the whole props object itself, so you destructure it into individual props.」（区块一的 OrderCard）
 *    - 【少用】整个 props 对象（props.amount）：见附 1。
 *    和 state 的分工 —— thinking-in-react：「Props are like arguments you pass to a function.」「State is like a component's memory.」（03 题）。
 *    传 props 就是写 JSX 属性：字符串可以直接用引号，其他 JS 值用花括号；属性名原样传给组件，JSX 不做大小写转换。
 *    只写属性名不写值等于传 true（<UiButton disabled> 编译成 { disabled: true }，出处见附 2）。写 disabled 还是 disabled={true}：两种都常见，按团队约定 ——
 *    Airbnb 的 React 风格指南要求省略（「Omit the value of the prop when it is explicitly true.」，对应 eslint-plugin-react 的 jsx-boolean-value 规则，默认选项 "never"；
 *    这条规则不在 eslint-plugin-react 的 recommended 里，要自己开），react.dev 自己的示例多写成 ={true}（learn/conditional-rendering 的 isPacked={true}、learn/typescript 的
 *    disabled={true}、reference/react-dom/components/input 的 defaultChecked={true} 等）。区块一的 emphasis 一行，测试覆盖。
 * 2. 默认值（区块一）：「The default value is only used if the size prop is missing or if you pass size={undefined}. But if you pass size={null} or size={0},
 *    the default value will not be used.」空串同理（测试覆盖）。按要做的事选写法：
 *    - 【最常用】参数里的解构默认值 { discount = 0 }（区块一）。TypeScript 里可选属性 discount?: number 解构出默认值之后，函数体里的类型就是 number。
 *    - 【常用】需要稳定引用的对象 / 数组默认值提到模块顶层常量：对象 / 数组 / 函数当默认值（{ items = [] }）每次渲染都是新的引用，传给 memo 子组件或当 Effect 依赖时
 *      会造成多余的更新（17 题）；写成 const EMPTY_ITEMS: Item[] = []，参数里写 { items = EMPTY_ITEMS }（工程经验）。
 *    - 【常用】null 也要走默认值：在函数体里写 const text = note ?? '（无备注）'（练习 2；工程经验：接口返回的 null 字段很常见）。
 *    - 函数组件的 defaultProps【旧写法】（19 起移除），见八。
 ...
 *    - 开发构建：createElement 页 Caveats「In development, React will freeze the returned element and its props property shallowly to enforce this.」函数组件拿到的 props
 *      就是 element.props 这个对象（源码位置见附 3），ES 模块是严格模式，所以 props.amount = 0 抛
 ...
 *    - 生产构建不冻结：赋值「成功」、不触发重渲染，之后被父组件的新 props 对象盖掉 —— 界面和数据源对不上（实测过程见附 3）。
 ...
 * 4. 子组件想改数据【主流】（区块二）：【最常用】单向数据流与回调上浮 —— 数据的主人（父组件）持有 state，把值和 onXxx 回调一起传下去；子组件调用回调「报告」想改成什么，
 *    改不改、怎么改由父组件决定（08 题）。依据：passing-props 上面那句「it will have to "ask" its parent component」、sharing-state-between-components 的状态提升（25 题）。
 *    隔了很多层时【常用】用 Context 往下传 dispatch / 回调，或者把数据放进 store（15、16 题，本题不展开）。
 ...
 *    - 【最常用】直接用 prop，派生值在渲染时算（区块三的 PriceViews；09 题）。
 *    - 【常用】有意只取初始值（编辑草稿、表单初始值）：「"Mirroring" props into state only makes sense when you want to ignore all updates for a specific prop. By convention,
 *      start the prop name with initial or default to clarify that its new values are ignored」。命名 initialX，要按新的初始值重来就换 key（区块三的 PriceDraft；06 题）（测试覆盖）。
 ...
 * 7. 接收原生属性【主流】（区块四）：类型上继承原生元素的 props 类型，运行时展开 rest。类型的写法（react.dev 没有讲这组工具类型，出处见附 7）：
 *    - 行业里【最常用】ComponentProps<'button'>：shadcn/ui 面向 React 19 的组件源码就这样写（apps/v4/registry/new-york-v4/ui/button.tsx、input.tsx 的 React.ComponentProps<"button">），
 *      它的 Tailwind v4 迁移文档也写着「Replace React.forwardRef<...> with React.ComponentProps<...>」。
 *    - 官方类型建议：@types/react 19.2.18 的 ComponentProps JSDoc「It's usually better to use ComponentPropsWithRef or ComponentPropsWithoutRef instead of this type, as they let you be explicit
 *      about whether or not to include the `ref` prop.」（:1430-1432）。本课 UiButton 按这个建议写 ComponentPropsWithRef<'button'>（统一措辞表定的主线）。
 *      对 DOM 元素这两个是同一个类型，里面都有 ref?: Ref<HTMLButtonElement> 和 key（测试覆盖类型层，练习 3 可以动手换）。
 *    - 要兼容 React 18 的组件库：forwardRef + ComponentPropsWithoutRef（WithoutRef 只去掉 ref，测试覆盖类型层）【旧写法】，存量代码里还很常见（例如 Radix UI 的
 *      packages/react/toggle/src/toggle.tsx）。见八。
 ...
 *    合并类名【最常用】用 clsx / cn 这类工具函数（01 题二-5 有 npm 下载量；shadcn/ui 的组件用 cn() 把外部 className 和自己的类名合在一起，button.tsx）；
 *    本课没装，UiButton 用 [a, b].filter(Boolean).join(' ') 演示简化。
 ...
 * 8. ref 作为 prop【主流·19.0 起】（区块四）：让自定义组件接收父组件的 ref，【最常用】把 ref 当普通 prop 收（不再需要 forwardRef）—— 解构出来显式交给 DOM 元素
 *    （react.dev 示例、本课 UiButton），或者留在 rest 里随展开一起交给元素（shadcn/ui），效果相同。
 *    react-19 博客「Starting in React 19, you can now access ref as a prop for function components」；「New function components will no longer
 ...
 *    instance.」区块四父组件的 ref 经 UiButton 转交，拿到的是真实的 <button>，可以直接 focus()（测试覆盖）。forwardRef 是 18 及以前的写法【旧写法】，见八。
 *    ref 回调的返回类型见附 5；ref 回调与清理函数、useImperativeHandle 12 题改写时补。
 * 9. children【主流】：嵌套在标签里的内容 ——「the parent component will receive that content in a prop called children」。类型：learn/typescript「There are two common paths to
 *    describing the children of a component.」【最常用】ReactNode：learn/typescript 先给的就是它，@types/react 自带的 PropsWithChildren 也是 children?: ReactNode（index.d.ts:1423）；
 *    13、28 题。【少用】只收 JSX 元素的 ReactElement：见附 8（两者的取舍 28 题改写时补）。
 * 10. 类型：interface 和 type 两种都常见，按团队约定 —— learn/typescript「you can use an interface or type to describe the component's props」；style 用 CSSProperties
 *    （「you can use React.CSSProperties to describe the object passed to the style prop」）。自定义 prop 和原生属性同名、类型又不兼容时先 Omit：<input> 自带 size?: number，
 *    想要 size?: 'sm' | 'md' 就写 Omit<ComponentPropsWithRef<'input'>, 'size'> & { size?: 'sm' | 'md' }；直接用 & 拼不报错，但 size 的类型变成 undefined，用 interface 继承
 *    则直接报错（测试覆盖类型层）。
 *
 * 三、Vue 对照（Vue 这一侧的演示都是 Vue 项目里常用的写法，没有注释掉的部分）
 * - 声明 props：【最常用】类型声明 defineProps<Props>()（编译器宏，不用 import）：typescript/composition-api「However, it is usually more straightforward to define props with
 *   pure types via a generic type argument」；【常用】运行时声明 defineProps({ amount: { type: Number, required: true, validator } })（props 页的主示例用的就是运行时声明，
 *   不写 TS 的项目只能用它；要自定义 validator 时也用它），
 *   「When prop validation fails, Vue will produce a console warning (if using the development build).」类型声明由编译器生成等价的运行时声明。
 *   React 的 props 就是函数参数，没有宏；React 19 删掉了 propTypes 检查，组件之间的 props 靠 TypeScript 这类静态类型检查（外部数据在边界用 schema 校验，见七）。
 * - 默认值【主流·3.5 起】：3.5 起的代码【最常用】响应式 props 解构 const { discount = 0 } = defineProps<Props>()（官方 TypeScript 指南 Props Default Values 一节先给的就是它，
 *   withDefaults 是给 3.4 及以前的 alternative）。props 页：「In version 3.5 and above, Vue's compiler automatically prepends props. when code
 *   in the same <script setup> block accesses variables destructured from defineProps.」解构出默认值之后，类型里的 undefined 也去掉了（编译结果与类型细节见附 6）。
 ...
 *   If you do, Vue will warn you in the console」。Vue 3.5.42 开发环境 setup 拿到的是 shallowReadonly(props)，赋值不抛错、值不变，
 *   警告「[Vue warn] Set operation on key "amount" failed: target is readonly.」（测试覆盖）；生产构建直接是可写的 props 对象，赋值会成功并触发重渲染（源码位置见附 6）。
 *   TypeScript 也拦：defineProps 的返回类型是只读的（vue/AmountEditor.vue 里要写 @ts-expect-error 才能演示）。【最常用】想改就 emit，交给父组件改（08 题）。
 ...
 * - props 是响应式对象【主流】：instance.props 是 shallowReactive，整个组件生命周期都是同一个对象。父组件重新渲染、把新值 patch 进来之后
 *   （下一个 tick），读 props.x 就是新值 —— 定时器里读到的是最新值，不是 React 那样「每次渲染一份快照」（区块二，测试覆盖）。
 * - 透传属性（fallthrough attributes）（区块四）：「A "fallthrough attribute" is an attribute or v-on event listener that is passed to a component, but is not explicitly declared in
 *   the receiving component's props or emits.」
 *   - 【最常用】单根组件什么都不用写：「When a component renders a single root element, fallthrough attributes will be automatically added to the root element's attributes.」
 *     class / style 与根元素已有的合并，监听器「both listeners will trigger」。
 *   - 【常用】defineOptions({ inheritAttrs: false })（defineOptions 3.3 起）+ useAttrs() / $attrs 手动接管，才相当于 React 的 {...rest}（区块四的 UiButton.vue 为了和 React 版逐行对照用的是这种）。
 *     什么时候用 —— 官方给的典型场景：「The common scenario for disabling attribute inheritance is when attributes need to be applied to other elements besides the root node.」
 *     （工程经验：包一层 <input> 的表单组件里常见）。
 ...
 *   纯 React 19 的组件用 WithRef（行业里常写成等价的 ComponentProps），ref 作为普通 prop（解构或随 rest 展开）；要兼容 React 18 的组件库用 forwardRef + WithoutRef。
 ...
 * - 同时支持 React 18 的组件库：forwardRef + ComponentPropsWithoutRef；只支持 19 的项目：ComponentPropsWithRef（或等价的 ComponentProps）+ ref 作为普通 prop（解构或随 rest 展开）。
 ...
 *   defaultProps since there is no ES6 alternative.」19.2.8 实测：日常 JSX（编译成 jsx()）下被静默忽略、没有任何提示；但某些写法会走 React.createElement、仍然合并 defaultProps，
 *   所以存量代码里的 defaultProps 时灵时不灵（机制见附 4，测试覆盖），迁移时统一改成参数默认值。
 ...
 * 3. 把 UiButton.tsx 的 import 和 UiButtonProps 里的 ComponentPropsWithRef 都换成 ComponentProps（shadcn/ui 的写法），跑 npm run typecheck 和本题测试：全部通过 —— 对 DOM 元素这两个是同一个类型。
 *    再换成 ComponentPropsWithoutRef。可断言：typecheck 报两处错 —— UiButton.tsx 里解构的 ref「Property 'ref' does not exist on type 'UiButtonProps'」，UiButtonDemo.tsx 里 ref={deleteRef} 不能赋值
 *    （WithoutRef 把 ref 去掉了，这就是 React 18 写法要配 forwardRef 的原因）。
 *
 * 附：少用的写法与细节（读别人的代码时认得出来就行；本题没有需要注释掉的演示）
 * 1. 【少用】整个 props 对象（function AmountEditor(props: Props)，读 props.amount）：官方说「Usually you don't need the whole props object itself」。区块二的 AmountEditor
 *    故意这样写，为了演示 props.amount = 0 这个反例；平时照样在参数里解构。需要把全部 props 往下传时用的是 ...rest（二-7），不是整个 props 对象。
 * 2. 布尔 prop 的出处：<UiButton disabled> 编译成 jsx(UiButton, { disabled: true })（esbuild 0.28.2 实测）。react.dev 没有写这条，出处是旧文档 legacy.reactjs.org 的 JSX In Depth：
 *    「If you pass no value for a prop, it defaults to true.」同一段也说「we don't recommend not passing a value for a prop, because it can be confused with the ES6 object shorthand {foo}」。
 *    这是同一时期的两种口径：React 旧文档（2023 年 react.dev 上线前的官方文档）不推荐省略；Airbnb 风格指南（2015 年起就有这条）和 eslint-plugin-react 的 jsx-boolean-value（默认 never）
 *    要求省略；react.dev 的示例多写 ={true}。所以写成「按团队约定」。
 * 3. props 冻结的源码与生产构建实测：开发构建 react/cjs/react-jsx-dev-runtime.development.js:193 Object.freeze(type.props)（开发环境的 jsxDEV；react-jsx-runtime.development.js 同一行号）。
 *    函数组件拿到的 props 就是 element.props（react-dom-client.development.js:5035-5042 createFiberFromElement 把 element.props 存成 pendingProps，:10130 updateFunctionComponent →
 *    :7662 renderWithHooks 里的 callComponentInDEV(Component, props) 原样传给组件函数）。生产构建不冻结（react-jsx-runtime.production.js 里没有 freeze）：赋值「成功」、不触发重渲染；
 *    这个组件之后因为自己的 state 重渲染时，读到的是改过的值（它拿到的还是同一个 props 对象）；父组件一重渲染，又换成新的 props 对象（node + react-dom 19.2.8 生产构建 + jsdom 实测）。
 * 4. defaultProps「时灵时不灵」的机制：React.createElement 仍会合并 type.defaultProps（react/cjs/react.development.js:1037-1039，不分函数和 class），而 <C {...p} key="k" />
 *    （key 写在展开后面）会被编译成 createElement（esbuild 实测）—— 测试覆盖：<Greeting {...p} key="k" /> 拿到了 defaultProps，<Greeting key="k" {...p} /> 没拿到。
 *    @types/react 19 的 FunctionComponent 已经没有 defaultProps 字段（index.d.ts:1060-1086）。
 * 5. ref 回调可以返回清理函数（19.0 起），升级指南：「Due to the introduction of ref cleanup functions, returning anything else from a ref callback will now be rejected by TypeScript.」
 *    （@types/react 19.2.18 index.d.ts:176-184 RefCallback 的返回类型是 void 或清理函数）。
 * 6. Vue 的细节：
 *    - 解构默认值的编译结果：@vue/compiler-sfc 3.5.42 实测，默认值被编进运行时 props 选项（discount: { type: Number, required: false, default: 0 }），后面对 discount 的访问改写成
 *      __props.discount；propsDestructure 在 3.5 默认开启（compiler-sfc.d.ts:119-124「@default true」，本项目 @vitejs/plugin-vue 6.0.8 没有另外配置）。
 *      解构出默认值之后类型里的 undefined 去掉了（vue-tsc 3.3.11 实测：OrderCard.vue 模板里的 discount > 0 能通过类型检查）。
 *    - 只读包装的源码：开发环境 setup 拿到 shallowReadonly(props)（runtime-core.cjs.js:8231-8239）；生产构建直接是可写的 props 对象（runtime-core.cjs.prod.js:6613）；
 *      instance.props 是 shallowReactive（runtime-core.cjs.js:4939-4940）；重复绑定时监听器按引用去重（mergeProps，:8012-8036）。
 *    - useAttrs 能不能 watch：实测 3.5.42 里 watch(() => attrs.title) 其实会触发 —— attrs 代理读的时候整体追踪（runtime-core.cjs.js:8365-8369 track(target, "get", "")），
 *      属性变化时 updateProps 统一触发（:5042），这套追踪是 3.2 为修「插槽里用 attrs 不更新」加的（3.2 CHANGELOG「ensure setupContext.attrs reactivity when used in child slots」#4161）；
 *      但 isReactive(attrs) 为 false，文档也没承诺这种行为，按文档写（测试记录了这个现状）。
 * 7. 原生属性类型的出处：react.dev 没有讲 ComponentProps 这组工具类型（learn/typescript 只讲了给自己的 props 写类型），依据是 @types/react 19.2.18 —— ComponentProps（index.d.ts:1452 起）、
 *    ComponentPropsWithRef（:1480-1484）、ComponentPropsWithoutRef（:1530）。WithRef 对非组件类型（'button' 这种标签名）直接返回 ComponentProps<T>，所以对 DOM 元素两者相同；
 *    ComponentProps<'button'> 就是 JSX.IntrinsicElements['button'] = ClassAttributes & ButtonHTMLAttributes（ClassAttributes 里带 ref 和 key）。
 * 8. 【少用】children 写成 ReactElement：「which is only JSX elements and not JavaScript primitives like strings or numbers」。只在要求子节点必须是一个元素的 API 里用
 *    （例如要 cloneElement 子元素、asChild 这类把 props 合到子元素上的组件，工程经验）；日常组件的 children 用 ReactNode。
 *
 * 参考（2026-09-19 核对，react.dev / vuejs.org 文档取自官方仓库原文）：
 * - react.dev：learn/passing-props-to-a-component、learn/thinking-in-react、learn/choosing-the-state-structure、learn/rendering-lists、learn/typescript、learn/conditional-rendering、
 ...
 * - 使用频率的依据：airbnb/javascript react/README.md（jsx-boolean-value）、jsx-eslint/eslint-plugin-react docs/rules/jsx-boolean-value.md、
 *   shadcn-ui/ui apps/v4/registry/new-york-v4/ui/button.tsx 与 input.tsx、apps/v4/content/docs/(root)/tailwind-v4.mdx（Remove forwardRef）、
 *   radix-ui/primitives packages/react/toggle/src/toggle.tsx（2026-09-19 取自 GitHub 仓库 main 分支）
