...
7. PROGRESS.md 的「**统一措辞**」一节（Vue 响应式、表单与事件类型、服务端状态 / TanStack Query、自定义 Hook 与订阅外部数据源、全局状态、错误边界、过期闭包与 useEffectEvent、Props、State、事件、条件渲染十一张表）：这些说法已定稿，后续各题直接照用，不要再自己另起一套。
