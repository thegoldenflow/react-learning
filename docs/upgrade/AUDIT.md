...
| **5.16（18 样板确认，2026-09-17）** | 用户答复「按你推荐的来」：PROGRESS.md 2.1 末尾的 6 项风格与深度全部确认（完整十段文件头只写在 react/Example.tsx、Vue 文件头精简并指向 React；一题多文件；主线能力做成可运行代码、并排只演示差异；关键结论配测试且与示例同目录；「（NN 题，待新增）」引用；模拟服务可注入延迟）。D2-8 的顺序微调：开头的批量修正只做「Vue 响应式措辞」一项（ref 不是 Proxy、更新单位是组件 render effect），「没有一一对应关系」残留和绝对化用词改为逐题清零、阶段 4 全局复查（每题都要整体重写，先批量改会重复劳动）。后续会话按 `docs/upgrade/CONTINUE-PROMPT.md` 执行 |
