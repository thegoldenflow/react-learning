...
**用户决定（2026-09-19）：保持 ComponentPropsWithRef，主线和统一措辞表都不改。** 当时的情况：行业里 React 19 组件最常见的原生属性类型写法是 `ComponentProps<'button'>`（shadcn/ui），本课主线 UiButton 按「统一措辞 · Props」表和 @types/react 的 JSDoc 写 `ComponentPropsWithRef<'button'>`。两者对 DOM 元素是同一个类型（测试覆盖）。按 RETROFIT §7.1 没有改主线和统一措辞表，只在正文分开写，汇报时问用户要不要把主线换成 ComponentProps。
...
### 2.20 04 按「使用频率」改写（2026-09-19）

**起因**：同 2.17，按 `RETROFIT-01-04-PROMPT.md` 改写。

**改动**
- 文件头：加「使用频率」说明行；30 秒速答只讲冒泡、stopPropagation / preventDefault 与表单 onSubmit；二-1 绑定（onClick={handleClick}、() => remove(id)【最常用】，(e) => remove(id, e)【常用】）；二-5 传播（onClick 冒泡【最常用】，onClickCapture【少用】）；二-6 stopPropagation（唯一写法）与 document 监听器的 { capture: true } 修法、表单 onSubmit【最常用】，链接 preventDefault【常用】；二-9 事件类型（具体类型【最常用】，SyntheticEvent / XxxEventHandler【常用】）；二-10 内联箭头【最常用】、useCallback 写「什么时候才需要」；新增二-11「Vue 修饰符的 JS 写法」；三的 Vue 修饰符按 Vue 项目频率分档（见下表）；源码行号、委托实现、Vue 修饰符实现挪进「附」；练习 1 改在 CaptureOrderDemo 上做，新增练习 3（取消区块六注释）；「附」1–5。
- 演示（按 RETROFIT §3.3「拆成单独的小组件再整体注释」）：
  - 区块三只留【最常用】的冒泡：按钮 → 中层 → 外层 + stopPropagation 勾选项 + document 上的两个监听器（冒泡阶段的被挡住、{ capture: true } 的照样收到 —— 点击外部关闭这类代码常碰到的坑与 17 RC 博客的修法）+ onScroll 不冒泡 / onFocus 冒泡；**onClickCapture、原生监听器混用顺序、document 监听、onScrollCapture 拆到新文件 `CaptureOrderDemo.tsx`**，页面上的 import 与用法用【少用】注释块包起来，组件文件保留、测试直接渲染它。
  - 区块五只留【最常用】按键 e.key（回车提交时排除 Shift【常用】、Esc【常用】、输入法组字判断）与【常用】state + disabled 的 .once；**ref 标记的 .once、.exact 精确组合键、鼠标按键与右键菜单拆到新文件 `RareModifiersDemo.tsx`**，同样注释用法、保留文件。
  - **区块六（PassiveWheelDemo）整块【少用】**：Example.tsx 里的 import 与用法注释，组件文件保留、测试直接渲染。
  - 区块一**补上【最常用】的 onClick={() => onRemove(item.id, true)}**（每行「减一件」按钮）；各区块补频率说明行。
- 测试：新增「减一件」、冒泡（含 document 冒泡 / 捕获监听）两条；原来渲染 PropagationDemo 的捕获顺序测试改成渲染 CaptureOrderDemo（名字加「附 1【少用】」），onScrollCapture 拆成单独一条；区块五拆成常用两条 + 附 2 三条（直接渲染 RareModifiersDemo）；区块六的测试名标「附 3【少用】」；整页测试改成 names 数组（注释块里 names.push('区块六')）。React 22 → 27 条。
- Vue 侧（按 Vue 项目里的频率判断，少用的同样注释）：区块三拆出 `vue/CaptureOrderDemo.vue`（.capture、原生监听器交错顺序、@scroll.capture），主演示改成冒泡 + .stop + document 两个监听器；区块五拆出 `vue/RareModifiersDemo.vue`（.ctrl.enter.exact 组合键精确区分、鼠标键与 @contextmenu.prevent），主演示留 .enter.exact（排除修饰键的回车，聊天框常用）、.esc、.once；区块六整块在 `Example.vue` 里注释；`OrderRow.vue` 补「减一件」（@click="emit('remove', item.id, true)"）。**.vue 的注释块约定**：模板里第一行以 `<!-- 【少用】` 开头并写明怎么取消、最后一行单独是 `-->`，中间不能再有 HTML 注释；script 里照旧用 `/* 【少用】` … `*/`。scratchpad 的 uncomment-rare.cjs 已扩展到 .vue。Vue 测试 15 → 19 条（新增冒泡 + document、@scroll.capture 拆出、组合键拆出、减一件；原来的顺序 / .stop / 鼠标键测试改成直接挂载拆出来的组件；整页测试改成 names 数组）。`Example.vue` 加使用频率说明、修饰符分档与「附：细节」。README 04 目录行与说明段、注册表 summary 更新。

**频率判断与依据**

| 要做的事 | 写法与标签 | 依据 |
|---|---|---|
| 绑定 | onClick={handleClick}、() => remove(id)【最常用】；(e) => remove(id, e)【常用】 | responding-to-events 的示例写法；工程经验 |
| 传播 | onClick 冒泡【最常用】；onClickCapture【少用】 | responding-to-events「Capture events are useful for code like routers or analytics, but you probably won't use them in app code.」 |
| 挡住外层 / 表单 | stopPropagation（唯一写法，不打频率标签）；form onSubmit + preventDefault【最常用】；链接 preventDefault【常用】；document 监听器改用 { capture: true } | responding-to-events；17 RC 博客「You can fix code like this by converting your listener to use the capture phase.」；工程经验 |
| 与原生监听器混用、stopImmediatePropagation、onScrollCapture | 【少用】 | 工程经验 |
| 按键 / 只能点一次 / .self | e.key + 组字判断【最常用】；回车排除 Shift、Esc、state + disabled、e.target !== e.currentTarget【常用】；ref 标记、组合键精确区分、鼠标键【少用】 | Vue event-handling「When listening for keyboard events, we often need to check for specific keys.」；MDN keydown；其余工程经验 |
| 拦截滚轮 | ref + addEventListener passive:false【少用】 | 工程经验（只有自定义缩放、横向轮播、拖拽） |
| 事件类型 | 从 react 导入具体类型【最常用】；SyntheticEvent、XxxEventHandler【常用】 | learn/typescript |
| 处理函数记忆化 | 内联箭头 / handleXxx 直接传【最常用】；useCallback 只写「什么时候才需要」（官方「only valuable in a few cases」），不标频率 —— 存量代码里到处包的写法很常见 | 工程经验 |
| Vue 修饰符 | .prevent .stop .enter【最常用】；.esc .self .once .enter.exact（排除修饰键的回车）系统键【常用】；.capture .passive、.ctrl.enter.exact 这类组合键精确区分、鼠标键【少用】 | event-handling「It is a very common need to call event.preventDefault() or event.stopPropagation() inside event handlers.」「we often need to check for specific keys」；其余工程经验 |

**注释掉了什么**：React 侧 Example.tsx 的 PassiveWheelDemo、PropagationDemo 的 CaptureOrderDemo、ModifiersDemo 的 RareModifiersDemo（各自 import + 用法）、整页测试里的 names.push('区块六')，共 7 个【少用】注释块；Vue 侧同样 7 个（Example.vue、PropagationDemo.vue、ModifiersDemo.vue 的 import + 用法，Example.test.ts 的 names.push）。区块六的 ❌「onWheel 里 preventDefault 拦不住」随场景一起注释（场景本身少用，文件头写明了这个例外）。取消两侧全部 14 个注释块后 lint、typecheck、04 测试（46 条）全绿（取消后页面上有两个 stopPropagation 勾选框，测试按名字区分），已从备份还原并 `diff -r` 一致。

**验证**：`npm run check` 通过（在只含 04 改动的暂存区导出目录里跑）；04 测试 46 条（React 27、Vue 19）；引文保留检查 85 / 0；浏览器（临时 5174，两侧各点一遍）：「减一件」把无线鼠标 × 2 变成 × 1；区块三日志是 document 捕获 → 按钮 → 中层 → 外层 → document 冒泡，勾上 stopPropagation 后只剩 document 捕获 → 按钮；区块五 Shift + Enter 不记、Enter 记「提交」；两侧都只有五个区块标题、没有捕获实验和滚轮区块；console.error / warn 为 0。
