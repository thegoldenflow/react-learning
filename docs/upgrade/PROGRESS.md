...
| 2 | 逐主题修改（先改 18 路由样板） | **进行中**（分支 `phase-2-topics`）：前置 commit（壳修复，2.0）、**18 路由样板**（2.1，风格已确认，AUDIT.md §5.0 的 5.16）、**2-A Vue 响应式措辞批量修正**（2.2，措辞见「统一措辞」）、**2-B 的 07 表单**（2.3）、**19 异步提交**（2.4）、**11 API 请求状态**（2.5）、**30 TanStack Query**（2.6）、**14 自定义 Hook**（2.7）、**16 全局状态**（2.8）、**20 错误边界**（2.9）、**26 过期闭包**（2.10）已完成，**2-B 全部完成**；2-C 的 **01 组件与 JSX**（2.11）、**02 Props**（2.12）、**03 State**（2.13）、**04 事件处理**（2.14）、**05 条件渲染**（2.15）已完成；2026-09-19 按用户要求定了「使用频率」写法，**05 按它改写成样板并经用户确认**（2.16）。2026-09-19 **01–04 已按使用频率改写**（2.17–2.20；fdbff97、9ee15d0、ade42f8 与 04 的提交）。**下一步：等用户定下一批** —— 18、07、19、11、30、14、16、20、26 按同样方式改写，还是先把 06 做完（06 要直接按使用频率写法完成）；另有一项待用户决定：02 题原生属性类型的主线要不要从 ComponentPropsWithRef 换成行业里更常见的 ComponentProps（2.18）。06 的半成品仍在 `git stash` 里（说明含 wip06），接续方法见 CONTINUE-PROMPT §3.3 | 每题一个 commit | 样板已确认，其余题不再逐题停 |
