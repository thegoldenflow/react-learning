...
>
> **执行状态（2026-09-19）：已完成（efe5615），不用再执行。** 记录见 PROGRESS 2.21（复核 36 条全部处理）；wip06 的 stash 已恢复并清空。用户随后选了先改写其余九题（18、07、19、11、30、14、16、20、26），执行 prompt 是 `docs/upgrade/RETROFIT-REST-PROMPT.md`。本文件保留作为「从半成品接着做、直接按使用频率写法完成」的记录。
