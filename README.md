...
| 26 | `26-stale-closures` | 过期闭包（stale closure） | 延迟回调、手动事件监听、轮询读到旧 state 的坏例子；修法按优先级：函数式更新 → 写对依赖 → `useEffectEvent`（主线，19.2 起）→ latest ref（并排）；让依赖合法消失（搬进事件、对象移进 Effect、`ref.current` 不能当依赖），对照 Vue 的现读 `.value` 与 `watch` / `watchEffect` |
...
**18 路由（React Router）** —— 课件升级阶段 2 的样板题（2026-09-17 重写，文件头按十段模板；2026-09-19 按使用频率改写：存量项目最常用声明式、官方给新项目推荐 Framework、本课主线 Data，三者分开写）。主线是 Data 模式：`createMemoryRouter` / `createBrowserRouter` 配置数组 + `RouterProvider`，loader / action / `errorElement` / `handle` / 路由级 `lazy`，登录守卫两种写法并排——分组路由 loader 里 `throw redirect`【最常用】（日志面板演示父子 loader 并行的坑）与 middleware【较新·7.9 起】【常用】（下游 loader 不执行）；`useFetcher` 星标（不跳转页面的提交、乐观显示）、`useNavigation`、`useBlocker`、`useMatches` 面包屑、`safeRedirect` 回跳校验。并排是声明式模式 + RequireAuth 三态（checking / authed / guest）。最重要的区别是拦截时机：渲染之前（Data 模式、Vue `beforeEach`）还是渲染之中（声明式 RequireAuth）；「路由表是组件树还是配置」只是声明式模式的表象。参数变化时组件实例复用、`setSearchParams` 整体替换、`navigate(-1)` 兜底、NavLink 默认 `active` 类都有测试（`react/Example.test.tsx`、`vue/Example.test.ts`）。Vue 侧用返回值写法的 `beforeEach` 与 memory history。

**19 异步提交与防重复** —— 主线是**手写 submitting**（`useState` 驱动界面 + `useRef` 锁 + `try / catch / finally`）：防重复三道关（按钮 `disabled` → 处理函数里的 state 守卫 → `useRef` 锁），结果用判别联合建模，错误分层（服务端字段错误显示在字段旁并聚焦、网络错误 `role="alert"` + 重试），成功清空、失败保留输入。页面上的「同一轮事件里提交两次」实验能看到：只有 state 守卫时两个请求都发出去了，加上 `useRef` 锁才拦住（渲染快照）。并排是 **React 19 Actions【较新·19.0 起】【常用】**（手写仍是最常用的写法）：`<form action>` + `useActionState`（`isPending`、重复提交排队串行）+ `useFormStatus`（按钮必须放在 `<form>` 的子组件里），并演示了一个常见坑——action 返回错误 state 也算成功，非受控字段照样被重置，要把提交的值放回 state 用 `defaultValue` 回填。Vue 侧逐行对应，但 `ref` 同步生效，守卫本身就够；事件处理函数里的同步错误和 async 拒绝都会进 `onErrorCaptured`（React 的错误边界接不住事件处理函数里的错误）。完整的 Actions（`useOptimistic`、Server Functions）在 31 题。
...
**26 过期闭包（stale closure）** —— 课件升级阶段 2 重写（2026-09-18，文件头按十段模板）。每次渲染的 props / state 是那次渲染的常量，回调活得比那次渲染长就读到旧值。四个区块各带运行日志：①「事件处理函数里的 setTimeout」：读快照 vs latest ref、`setCount(count + 1)` 倒退 vs 函数式更新 —— 这里 `useEffectEvent` 用不了（只能在 Effect 里调用），正是 19.2 之后仍要 latest ref 的场景；②「手动 `addEventListener`」：依赖 `[]`、写对依赖（每次都重注册）、`useEffectEvent`（只注册一次）、JSX 事件，以及被 `useCallback(fn, [])` 缓存后同样过期的 JSX 处理函数；③「轮询」四种写法并排：❌ `[]` / ② `[status]` 重建定时器 / ③ `useEffectEvent`【较新·19.2 起】主线 / ④ latest ref 并排（React 18 也能用），真实项目的轮询交给 TanStack Query 的 `refetchInterval`（30 题）；④「让依赖合法消失」：交互逻辑搬回事件处理函数、每次渲染新建的对象当依赖、`ref.current` 写进依赖数组没用。测试还证明了 Effect Event 没有稳定身份但会转发到最新回调、渲染期调用会抛错，以及用 `useEffect` 同步的 latest ref 有窗口期（layout effect 和子组件的 Effect 读到旧值，Effect Event 没有）。Vue 侧：现读 `.value` 默认不过期，但手动快照、解构 `reactive` 一样会过期（`toRefs` 修复）；3.5 解构 props 由编译器改写成 `props.x`；轮询用一个 `watch` + `onWatcherCleanup`；`watchEffect` 追踪回调里读到的一切、`watch` 只追踪 source（概念上对应 `useEffectEvent`）。React 22 条、Vue 15 条结论测试（`react/Example.test.tsx`、`vue/Example.test.ts`）。
...
| 26 | 什么是过期闭包，是 bug 还是设计？`setInterval(() => setCount(count + 1), 1000)` 配 `[]` 有什么问题、至少三种修法？`useEffectEvent` 解决什么、有哪些限制、和 `useCallback` 各管什么？为什么 `ref.current` 不能当依赖、19.2 之后还需要 latest ref 吗？`exhaustive-deps` 报警时按什么顺序处理？Vue 为什么默认不过期、什么写法也会过期？ |
